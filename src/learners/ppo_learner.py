    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, task_id=None):
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            for key in ["critic_loss", "critic_grad_norm", "td_error_abs", "q_taken_mean", "target_mean"]:
                train_stats[f"train/{key}"] = sum(critic_train_stats[key]) / ts_logged
                self.logger.log_stat(key, sum(critic_train_stats[key]) / ts_logged, t_env)
            train_stats["train/advantage_mean"] = (advantages * mask).sum().item() / mask.sum().item()
            train_stats["train/pg_loss"] = pg_loss.item()
            train_stats["train/agent_grad_norm"] = grad_norm.item()
            train_stats["train/pi_max"] = (pi.max(dim=-1)[0] * mask).sum().item() / mask.sum().item()
