...
class MADDPGLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, task_id=None):
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            train_stats = {}
            train_stats["train/critic_loss"] = loss.item()
            train_stats["train/critic_grad_norm"] = critic_grad_norm.item()
            train_stats["train/agent_grad_norm"] = agent_grad_norm.item()
            mask_elems = mask.sum().item()
            train_stats["train/td_error_abs"] = masked_td_error.abs().sum().item()
            train_stats["train/q_taken_mean"] = (q_taken).sum().item() / mask_elems
            train_stats["train/target_mean"] = targets.sum().item() / mask_elems
            train_stats["train/pg_loss"] = pg_loss.item()
            train_stats["train/agent_grad_norm"] = agent_grad_norm
