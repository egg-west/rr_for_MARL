...
class TaskEncoderMAC:
    ...
    def select_actions(
        ...
        agent_outputs, _ = self.forward(ep_batch, t_ep, test_mode=test_mode, agent_embedding=agent_embedding)
    ...
    def forward(self, ep_batch, t, test_mode=False, agent_embedding=None):
        ...
        observations = observations.reshape((-1, observations.shape[-1])) # [bs*n_agents, obs_dim]
        # // self.args.n_agents
        agent_id = th.eye(self.args.n_agents).to(self.args.device)# [2, 2]
        agent_id = agent_id.unsqueeze(0).expand(bs, -1, -1)
        agent_id = agent_id.reshape(bs*self.args.n_agents, -1)
        observations = th.cat([observations, agent_id], dim=1)
        ...
        #return agent_outs.view(bs, self.n_agents, -1), None
        return agent_outs.view(bs, self.n_agents, -1), self.hidden_states
    ...
    def _build_inputs(self, batch, t, agent_embedding=None):
        ...
        if self.args.obs_last_action:
            if t == 0:
                ...
                inputs.append(th.zeros_like(actions_onehot[:, t]))
            else:
                #inputs.append(batch["actions_onehot"][:, t-1])
                inputs.append(actions_onehot[:, t-1])
    ...
    def aux_encode(self, observations):
        return self.agent.aux_encode(observations)
